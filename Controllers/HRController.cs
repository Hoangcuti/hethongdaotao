...
public class HRController : Controller
{
    ...
    private IActionResult? RequireTrainingCenterManagerApi()
    {
        ...
        var role = HttpContext.Session.GetString("Role");
        if (role != "Manager" || !IsTrainingCenter())
            return StatusCode(StatusCodes.Status403Forbidden, new { error = "Chỉ Trưởng phòng Đào tạo mới có quyền thực hiện thao tác này." });
    ...
    public async Task<IActionResult> CreateAssignment([FromBody] CreateAssignmentDto dto)
    {
        ...
        if (course == null) return NotFound(new { error = "Khóa học không tồn tại." });

        if (course.EndDate.HasValue && course.EndDate.Value.Date < DateTime.Today)
        {
            return BadRequest(new { error = "Khóa học này đã kết thúc, không thể phân công." });
        }

        DateTime? finalDueDate = dto.DueDate;
        if (!finalDueDate.HasValue)
        {
            finalDueDate = course.EndDate;
        }
        else
        {
            if (course.EndDate.HasValue && finalDueDate.Value.Date > course.EndDate.Value.Date)
            {
                return BadRequest(new { error = $"Hạn hoàn thành không được muộn hơn ngày kết thúc của khóa học ({course.EndDate.Value:dd/MM/yyyy})." });
            }
            if (finalDueDate.Value.Date < DateTime.Today)
            {
                return BadRequest(new { error = "Hạn hoàn thành không được sớm hơn ngày hiện tại." });
            }
        }
    ...
    public async Task<IActionResult> GetEmployeeProfile(int id)
    {
        ...
        var groupedAssignments = assignmentsList
            .Where(ta => ta.CourseId.HasValue)
            .GroupBy(ta => ta.CourseId.Value)
            .Select(g => g.OrderByDescending(ta => ta.DueDate ?? DateTime.MaxValue).First())
            .ToList();

        foreach (var ta in groupedAssignments)
        {
    ...
    public async Task<IActionResult> DeleteAssignment(int id)
    {
        ...
        var allRelated = await _db.TrainingAssignments
            .Where(ta => ta.UserId == assignment.UserId && ta.CourseId == assignment.CourseId)
            .ToListAsync();
        _db.TrainingAssignments.RemoveRange(allRelated);
        await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> ExtendAssignment(int id, [FromBody] ExtendAssignmentDto dto)
    {
        ...
        {
            return BadRequest(new { error = "Vui lòng chọn thời gian gia hạn hợp lệ." });
        }

        var newDueDateLocal = dto.NewDueDate.Value.Kind == DateTimeKind.Utc
            ? dto.NewDueDate.Value.ToLocalTime()
            : dto.NewDueDate.Value;

        var maxAllowedDate = DateTime.Now.AddHours(72).AddMinutes(5);
        if (newDueDateLocal > maxAllowedDate)
        {
            return BadRequest(new { error = "Thời gian gia hạn không được vượt quá 3 ngày (72 giờ) kể từ hiện tại." });
        }
        if (newDueDateLocal < DateTime.Now.AddMinutes(-5))
        {
            return BadRequest(new { error = "Thời gian gia hạn phải lớn hơn thời gian hiện tại." });
        }

        var allRelated = await _db.TrainingAssignments
            .Where(ta => ta.UserId == assignment.UserId && ta.CourseId == assignment.CourseId)
            .ToListAsync();
        foreach (var ta in allRelated)
        {
            ta.DueDate = newDueDateLocal;
        }
        await _db.SaveChangesAsync();

        // Gửi thông báo cho học viên biết họ đã được gia hạn
        var course = await _db.Courses.FindAsync(assignment.CourseId);
        var notifTitle = $"Bạn đã được gia hạn khóa học '{course?.Title}' đến ngày {newDueDateLocal.ToString("dd/MM/yyyy HH:mm")}.";
    ...
    public async Task<IActionResult> GetCourses(string? search)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            query = query.Where(c => c.Title.ToLower().Contains(clean) || (c.CourseCode != null && c.CourseCode.ToLower().Contains(clean)));
            pendingQuery = pendingQuery.Where(d => d.Title != null && d.Title.ToLower().Contains(clean));
        }
    ...
    public async Task<IActionResult> DeleteDeptCourse(int id)
    {
        ...
        {
            return StatusCode(500, new { error = "Lỗi khi xóa khóa học và dữ liệu liên quan: " + ex.Message });
        }
    }
    ...
    [HttpGet("/api/hr/documents/{id}")]
    public async Task<IActionResult> GetDocumentById(int id)
    {
        var auth = RequireManager();
        if (auth != null) return auth;

        var doc = await (from d in _db.DocumentLibraries
                          where d.Id == id
                          join c in _db.Courses on d.CourseId equals c.CourseId into cj
                          from c in cj.DefaultIfEmpty()
                          join m in _db.CourseModules on d.ModuleId equals m.ModuleId into mj
    ...
    public async Task<IActionResult> UploadDocument([FromBody] UploadDocDto dto)
    {
        ...
        try {
            ...
            // Send notification to Training Center Manager
            try {
                var trainingDept = await _db.Departments.FirstOrDefaultAsync(d => d.DepartmentName == "Trung tâm Đào tạo Nội bộ");
                if (trainingDept != null && trainingDept.ManagerId.HasValue)
                {
                    var senderUser = await _db.Users.FindAsync(userId);
                    var senderName = senderUser?.FullName ?? senderUser?.Username ?? "Trưởng bộ phận";
                    
                    var prefix = $"[ĐỀ XUẤT - ID:{doc.Id}] ";
                    var body = $"[{senderName}] đã gửi đề xuất mới: '{doc.Title}'. Vui lòng xem xét phê duyệt.";
                    var notifTitle = prefix + body;
                    if (notifTitle.Length > 255)
                    {
                        notifTitle = notifTitle.Substring(0, 252) + "...";
                    }
                ...
                // Prevent notification errors from breaking the main document upload flow
            }
    ...
    public async Task<IActionResult> GetStaffProgress()
    {
        ...
        foreach (var u in users)
        {
            ...
            foreach (var exam in exams)
            {
                ...
                {
                    var ta = userAssignments
                        .Where(a => a.CourseId == exam.CourseId)
    ...
    public async Task<IActionResult> CreateExam(int courseId, [FromBody] ItCreateExamDto dto)
    {
        ...
        if (dto.PassScore < 0)
            return BadRequest(new { error = "Điểm đỗ của quiz không được phép âm." });

        if (dto.MaxAttempts.HasValue && dto.MaxAttempts.Value < 0)
            return BadRequest(new { error = "Số lần làm bài tối đa không được phép âm." });

        if (dto.DurationMinutes < 0)
            return BadRequest(new { error = "Thời gian làm bài không được phép âm." });

        var vnNow = DateTime.UtcNow.AddHours(7);
        if (dto.StartDate.HasValue)
        {
            if (dto.StartDate.Value < vnNow.AddMinutes(-5))
            {
                return BadRequest(new { error = "Ngày bắt đầu không được ở trong quá khứ." });
            }
        }

        if (dto.EndDate.HasValue)
        {
            if (dto.EndDate.Value < vnNow.AddMinutes(-5))
            {
                return BadRequest(new { error = "Ngày kết thúc không được ở trong quá khứ." });
            }
        }

        if (dto.StartDate.HasValue && dto.EndDate.HasValue && dto.EndDate.Value < dto.StartDate.Value)
        {
            return BadRequest(new { error = "Ngày kết thúc không được nhỏ hơn ngày bắt đầu." });
        }
    ...
    public async Task<IActionResult> GetContentLibrary()
    {
        ...
        var modules = await _db.CourseModules
            ...
            .Include(m => m.Course)
            .Include(m => m.Lessons)
            .Where(m => m.Course == null || m.Course.Status != "Deleted")
        ...
        var lessons = await _db.Lessons
            ...
            .Where(l => l.Module == null || l.Module.Course == null || l.Module.Course.Status != "Deleted")
        ...
        var exams = await _db.Exams
            ...
            .Include(e => e.Course)
            .Include(e => e.ExamQuestions)
            .Where(e => e.Course == null || e.Course.Status != "Deleted")
    ...
    public async Task<IActionResult> UpdateExam(int examId, [FromBody] ItCreateExamDto dto)
    {
        ...
        var vnNow = DateTime.UtcNow.AddHours(7);
        if (dto.StartDate.HasValue && (!exam.StartDate.HasValue || Math.Abs((dto.StartDate.Value - exam.StartDate.Value).TotalSeconds) > 1))
        {
            if (dto.StartDate.Value < vnNow.AddMinutes(-5))
            {
                return BadRequest(new { error = "Ngày bắt đầu không được ở trong quá khứ." });
        ...
        if (dto.EndDate.HasValue && (!exam.EndDate.HasValue || Math.Abs((dto.EndDate.Value - exam.EndDate.Value).TotalSeconds) > 1))
        {
            if (dto.EndDate.Value < vnNow.AddMinutes(-5))
            {
                return BadRequest(new { error = "Ngày kết thúc không được ở trong quá khứ." });
    ...
    // ============================================================
    // API: APPROVALS (PHÊ DUYỆT TÀI LIỆU) - PHÒNG ĐÀO TẠO
    // ============================================================
    [HttpGet("/api/hr/approvals")]
    public async Task<IActionResult> GetApprovals()
    {
        var auth = RequireTrainingCenterManagerApi();
        if (auth != null) return auth;
    ...
    [HttpPost("/api/hr/approvals/{id}/approve")]
    public async Task<IActionResult> ApproveDocument(int id)
    {
        ...
        if (!string.IsNullOrWhiteSpace(doc.PendingData))
        {
            ...
            {
                var data = JsonSerializer.Deserialize<HrCreateModuleDto>(doc.PendingData, options);
                if (data != null)
                {
            ...
            {
                var data = JsonSerializer.Deserialize<PendingLessonData>(doc.PendingData, options);
                if (data != null)
                {
            ...
            {
                var data = JsonSerializer.Deserialize<PendingExamData>(doc.PendingData, options);
                if (data != null)
                {
    ...
    [HttpPost("/api/hr/approvals/{id}/reject")]
